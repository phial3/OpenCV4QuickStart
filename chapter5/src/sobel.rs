...
pub(crate) fn run() -> Result<()> {
    // 读取图像，黑白图像边缘检测结果较为明显
    let img = imgcodecs::imread(&format!("{}{}", BASE_PATH, "equalLena.png"), imgcodecs::IMREAD_ANYCOLOR)?;
    if img.empty() {
        panic!("请确认图像文件名称是否正确");
    }
    ...
    // 整幅图像的一阶边缘
    opencv::core::add(&result_x_abs, &result_y_abs, &mut result_xy, &opencv::core::no_array(), -1)?;
